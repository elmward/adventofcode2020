ACTIVE = '#'
Point = Struct.new("Point", :x, :y, :z, :w)

def adjacencies(x, y, z, w, active_points)
  adj = 0
  (-1..1).each do |dx|
    (-1..1).each do |dy|
      (-1..1).each do |dz|
        (-1..1).each do |dw|
          ...
          adj += 1 unless active_points[Point.new(x+dx, y+dy, z+dz, w+dw)].nil?
        end
      end
    end
  end
...
def iterate(active_points)
  ...
  min_w = active_points.keys.min_by { |pt| pt.w }.w - 1
  max_w = active_points.keys.max_by { |pt| pt.w }.w + 1
      ...
      (min_y..max_y).each do |y|
        (min_z..max_z).each do |z|
          (min_w..max_w).each do |w|
            pt = Point.new(x, y, z, w)
            active = active_points[pt] == ACTIVE
            adj = adjacencies(x, y, z, w, active_points)
  ...
  points = {}.tap do |initial_points|
    rows.each_with_index do |row, y|
      row.chars.each_with_index do |cell, x|
        initial_points[Point.new(x, y, 0, 0)] = ACTIVE if cell == ACTIVE
      end
    end
  end
